...
export default function Table3() {
  ...
  useEffect( () => {
    async function fetching () {
      try {
        const rows = [];
        const response = await axios.get('https://node.ecell-iitkgp.org/comp/getAllDataOfTable?tableName=finlatics_reg_iit');
        response.data.map(e => {
          rows.push(createData(e.name_iit,e.roll,e.email_iit,e.mobile_iit));
        })
        setRow(rows);
      } catch (error) {
        console.error(error);
