...
const columns = [
  ...
  { id: 'ges_id', label: 'GES-ID', minWidth: 100 },
  {
    id: 'email',
    label: 'Email',
    minWidth: 170,
...
function createData(name, ges_id, email, mobile,email2, email3, email4,email5) {
  return { name, ges_id, email, mobile, email2, email3, email4,email5 };
}
...
export default function Table2() {
  ...
  useEffect( () => {
    async function fetching () {
      try {
        const rows = [];
        const response = await axios.get('https://node.ecell-iitkgp.org/comp/getAllDataOfTable?tableName=deep_learning_reg');
        response.data.map(e => {
